...
server.py
---------
Feedstock Advisor MCP Server — entry-point.

Registers all 15 tools and starts the stdio transport loop.

Tools registered
----------------
Helpers (7)
  list_available_crudes        – all crude names grouped by region
  list_available_refineries    – all refineries with id/name/location
  list_properties_and_yields   – canonical property & yield names + units
  find_matching_crude          – fuzzy-match a user-typed crude name
  find_matching_property       – fuzzy-match a property or yield name
  find_matching_refinery       – fuzzy-match a refinery name / location
  get_refinery_id              – exact name → refinery ID lookup

Forecasting (1)
  forecast_tool                – price / properties / yield forecast
                                 for a single crude in a target month

Blending (1)
  blend_simulator_tool         – simulate blending ≥2 crudes;
                                 blended properties + product yields

Analysis (6)
  constraint_checker_tool      – check blend vs property constraints
  price_calculator_tool        – total delivered cost of a crude blend
  profitability_calculator_tool – gross profit, margin, ROI
  optimizer_tool               – LP-based optimal blend
  get_high_yield_crudes        – rank crudes by yield, filtered by max sulfur
  get_market_context           – demand trend, sentiment, buy/hold signal
...
# ---------------------------------------------------------------------------
# Shared data store  (all 18 CSVs loaded once at startup)
# ---------------------------------------------------------------------------
...
def get_market_context_tool(
    ...
    RECOMMENDATION SIGNALS:
        BUY     — demand Increasing AND price Bullish/Neutral with positive sentiment
        CAUTION — demand Decreasing OR price Bearish
        HOLD    — otherwise (Stable / Neutral)

    USAGE:
        - Use to get the market buy/hold signal before purchasing a crude.
        - Combine with forecast_tool_mcp for a complete buy/hold recommendation.

    RELATED TOOLS:
        forecast_tool_mcp, profitability_calculator_tool_mcp
    """
...
if __name__ == "__main__":
    logger.info("Starting Feedstock Advisor MCP Server (stdio transport)…")
